...
const sampleBudget: BudgetData<BudgetSettings> = {
  id: 'budget-1',
  name: 'Test Budget',
  year: 2026,
  paySettings: {
...
describe('budgetCurrencyConversion', () => {
  ...
  describe('Round-trip conversion accuracy', () => {
    ...
    it('USD → JPY → USD: $65,000 salary round-trip with realistic rates', () => {
      ...
      const jpyToUsdRate = 1 / usdToJpyRate;
    ...
    it('Precision degradation with manual inverse vs automatic: 8-decimal vs 2-decimal', () => {
      const rate = 0.849577;
      const manualInverse = 1.177;
      const preciseInverse = 1 / rate;
      ...
      expect(manualBack.paySettings.annualSalary).toBe(64996.89);
      ...
      expect(preciseLoss).toBeLessThan(0.02);
    });
  });
});
